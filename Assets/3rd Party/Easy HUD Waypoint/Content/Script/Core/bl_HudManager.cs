    ...
    void OnScreen(int i)
    {
        ...
        if ((Vector3.Dot(this.LocalPlayer.forward, RelativePosition - this.LocalPlayer.position) > 0f))
        {
            ...
            else
            {
                ...
                {
                    GUI.Label(new Rect((drawPosition.x - Huds[i].m_Text.Length), (drawPosition.y - n) - 35f, 300, 60),"<color=whitte>[" + string.Format("{0:N0}m", CompleteDistance) + "]</color>", TextStyle);
                }
            }
        }
    }
            ...
            if (!bl_HudUtility.isOnScreen(bl_HudUtility.ScreenPosition(Huds[i].m_Target), Huds[i].m_Target))
            {
              ...
              //Get pivot from area
                ...
                else
                {
                    ...
                    {
                        GUI.Label(bl_HudUtility.ScalerRect(new Rect(mPivot.x + marge.x, mPivot.y + marge.y, 300, 75)),"<color=whitte>[" + string.Format("{0:N0}m", Distance) + "]</color>", TextStyle);
                    }
                }
